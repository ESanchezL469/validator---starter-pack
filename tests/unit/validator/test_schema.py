import pytest
import pandas as pd
from app.validators.schema import DataframeValidator  # ajusta el import según tu estructura
...
def test_validate_unique(sample_df):
    ...
    assert validator.violations[0]["invalid_count"] == 2  # incluye duplicado y None

def test_apply_rules(sample_df):
    rules = [
        {"column": "name", "rule": "not_null"},
        {"column": "age", "rule": "range", "min": 18, "max": 99},
        {"column": "email", "rule": "regex", "pattern": r"^[^@\s]+@[^@\s]+\.[^@\s]+$"},
