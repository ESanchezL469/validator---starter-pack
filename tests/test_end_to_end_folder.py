...
def test_full_pipeline_folder(tmp_path):
...
def test_full_pipeline_folder_profile(tmp_path):
    ...
    result = subprocess.run(
        ["python", "app/main.py", "--input-folder", str(input_dir),'--profile'],
        capture_output=True,
        text=True,
        env=env
    )
    ...
    assert len(glob.glob('reports/*_report.txt')) == 2, "No report files were created."
    assert len(glob.glob('metadata/*_metadata.json')) == 2, "No metadata files were created."
    assert len(glob.glob('profiles/*_profile.html')) == 2, "No profile report files were created."